...
# Base de datos con Coeficiente de Volatilidad integrado
PARTIDOS_DATA = [
    {
        ...
        "pick_seguro": "Brasil vs Noruega ➔ Más de 1.5 Goles", "pick_prob": 0.95, "volatilidad_arbitro": "BAJA"
    },
    {
        ...
        "pick_seguro": "México vs Inglaterra ➔ Más de 3.5 Tarjetas", "pick_prob": 0.94, "volatilidad_arbitro": "BAJA"
    },
    {
        ...
        "pick_seguro": "Portugal vs España ➔ Ambos Anotan Gol", "pick_prob": 0.93, "volatilidad_arbitro": "BAJA"
    }
]

@app.route('/api/live-data')
def api_live_data():
    # FILTRO DE SEGURIDAD EXTREMA: Solo entran picks con probabilidad individual >= 92%
    UMBRAL_SEGURIDAD = 0.92
    exito_combinado = 1.0
    lineas_filtradas = []
    
    for p in PARTIDOS_DATA:
        # El algoritmo descarta automáticamente cualquier picks volátil o dudoso
        if p["pick_prob"] >= UMBRAL_SEGURIDAD and p["volatilidad_arbitro"] == "BAJA":
            exito_combinado *= p["pick_prob"]
            lineas_filtradas.append(f"🔒 {p['pick_seguro']} ({int(p['pick_prob']*100)}% Fiabilidad)")
            
    porcentaje_mejorado = f"{round(exito_combinado * 100, 1)}%"
    
    return jsonify({
        "partidos": PARTIDOS_DATA,
        "combinada_picks": lineas_filtradas,
        "combinada_total": porcentaje_mejorado,
...
def home():
    ...
    <head>
        ...
        <title>CLAYTON LIVE FILTERED</title>
        <style>
            ...
            .premium { background: linear-gradient(135deg, #111827, #064e3b); border: 2px solid var(--green); border-radius: 12px; padding: 12px; margin-bottom: 14px; font-size: 11px; }
    ...
    <body>
        <header><h3 style="margin:0;">🧠 CLAYTON ALGORITHM TUNED</h3></header>
        
        <!-- PANEL DE COMBINADA CON FILTRO OPTIMIZADO -->
        <div class="premium">
            <b style="color:var(--green); font-size:12px;">🛡️ SUPERCOMBINADA OPTIMIZADA (FILTRO DE ALTA SEGURIDAD ACTIVADO)</b>
            ...
            <div style="margin-top: 8px; font-weight: bold; color: var(--gold); border-top: 1px dashed #2d3748; padding-top: 6px; text-align: center;">
                📈 FIABILIDAD MATEMÁTICA DEL FILTRO: <span id="combinada-total" style="font-size:14px; color:var(--green);">0%</span>
            </div>
        </div>
            ...
            async function actualizarPartidos() {
                ...
                data.partidos.forEach(p => {
                    ...
                    <div class="panel">
                        ...
                        <div class="grid">
                            <div class="box"><div class="lbl">🔮 Marcador Más Viable</div><div class="val c-gold">${p.resultados_exactos[0].score}</div></div>
